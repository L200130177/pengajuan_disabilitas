<?php
class Cloud_storrage_m extends CI_Model{
    ...
    {
        $data = array(
            "ref_folder"    => $post['date'],
            "ref_owner"     => $post['inputor'],
            "ref_file1"     => $post['file_name1'],
            "ref_file2"     => $post['file_name2'],
        );
        $this->db->insert('storrage', $data);
    }


	public function get($nik = null)
    {
        $this->db->from('storrage');
        if($nik != null) {
